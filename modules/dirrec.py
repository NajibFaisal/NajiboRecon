...
async def run(target, threads, tout, wdlist, redir, sslv, dserv, output, data, filext):
    ...
    conn = aiohttp.TCPConnector(
        limit=threads, resolver=resolver, family=socket.AF_INET, verify_ssl=sslv)
    timeout = aiohttp.ClientTimeout(
        total=None, sock_connect=tout, sock_read=tout)
            ...
            with open(wdlist, 'r') as wordlist:
                for word in wordlist:
                    ...
                    task = asyncio.create_task(
                        fetch(url.format(word), session, redir, sslv))
                    tasks.append(task)
...
async def wayback(target, dserv, tout):
    ...
    print('\n' + Y + '[!]' + C +
          ' Checking Availability on Wayback Machine' + W, end='')
        ...
        if entry != None:
            if entry[1] in {200}:
                if str(entry[0]) != target + '/':
                    ...
                    print(G + '[+]' + G + ' {}'.format(str(entry[1]
                                                           ) + C + ' | ' + W + '{}'.format(entry[0])))
            elif entry[1] in {301, 302, 303, 307, 308}:
                found.append(entry[0])
                print(G + '[+]' + Y + ' {}'.format(str(entry[1]) +
                      C + ' | ' + W + '{}'.format(entry[0])))
            elif entry[1] in {403}:
                found.append(entry[0])
                print(G + '[+]' + R + ' {}'.format(str(entry[1]) +
                      C + ' | ' + W + '{}'.format(entry[0])))
            else:
...
def dir_output(output, data):
    ...
    for entry in responses:
        if entry != None:
            if entry[1] in {200}:
                ...
                if output != 'None':
                    result.setdefault('Status {}'.format(
                        str(entry[1])), []).append(entry[0])
            elif entry[1] in {403}:
                if output != 'None':
    ...
    print(G + '[+]' + C + ' Total Requests      : ' +
          W + str(len(found) + len(skipped)))
    print(G + '[+]' + C + ' Directories Found on Wayback Machine : ' +
          W + str(len(wayback_found)))
    ...
    if output != 'None':
        ...
        result['Directories Found on Wayback Machine'] = str(
            len(wayback_found))
...
def hammer(target, threads, tout, wdlist, redir, sslv, dserv, output, data, filext):
    ...
    loop.run_until_complete(
        run(target, threads, tout, wdlist, redir, sslv, dserv, output, data, filext))
    filter_out(target)
