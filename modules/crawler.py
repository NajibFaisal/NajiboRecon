...
async def robots(target):
    ...
    try:
        r_rqst = requests.get(r_url, headers=user_agent,
                              verify=False, timeout=10)
...
def out(target, output, data):
    ...
    print('\n' + G + '[+]' + C +
          ' Total Unique Links Extracted : ' + W + str(len(total)))
    ...
    if output != 'None':
        if len(total) != 0:
            ...
            if len(js_crawl_total) != 0:
                data['module-Crawler'].update(
                    {'Links inside Javascripts': list(js_crawl_total)})

            if len(sm_crawl_total) != 0:
                data['module-Crawler'].update(
                    {'Links Inside Sitemaps': list(sm_crawl_total)})

            if len(int_total) != 0:
                data['module-Crawler'].update(
                    {'Internal Links': list(int_total)})

            if len(ext_total) != 0:
                data['module-Crawler'].update(
                    {'External Links': list(ext_total)})
            ...
            if len(wayback_total) != 0:
                data['module-Crawler'].update(
                    {'Wayback Machine': list(wayback_total)})
